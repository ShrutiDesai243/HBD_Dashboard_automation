...
def fetch_flipkart_data():
    try:
        ...
        query = Flipkart.query.order_by(Flipkart.id.desc())
        
        if search:
            query = query.filter(Flipkart.product_name.ilike(f"%{search}%"))
        if category:
            query = query.filter(Flipkart.leaf_category.ilike(f"%{category}%"))
