...
const ListingIncomplate = () => {
  ...
  const fetchIncompleteData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Incomplete_Listings");
    XLSX.writeFile(wb, `Listing_Incomplete_Page_${currentPage}.xlsx`);
  };
        ...
        <div>
          ...
          <Typography variant="small" className="font-medium text-gray-500">
            {error ? (
              ...
              `Displaying records requiring verification (${totalRecords} total)`
            )}
          </Typography>
        </div>
            ...
            <table className="w-full min-w-[1500px] table-fixed text-left">
              ...
              <tbody>
                {pageData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={incompleteColumns.length} className="p-20 text-center">
                      <Typography variant="h6" color="blue-gray" className="opacity-40 italic">
                        {error || "No incomplete records found"}
                      </Typography>
                    </td>
                  </tr>
                )}
